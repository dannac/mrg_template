---
title: "02_mpa_guellec_adults_simdig_2"
output: html_document
---

## Corresponds to paper in MMF -> literature -> 02_Model
## Originally Model_MPA_02.Rmd in IMMS -> lab-notebook -> MMF 

## ** Population pharmacokinetics and Bayesian estimation of MPA concentration in stable renal transplant patients **
## Chantal Le Guellec
## 2004, Clin Pharmacokinet


### Trial Design
* Retrospective study, 70 renal transplant patients
* Dose: MMF 1000mg PO Q12H. MMF was administered under fasting conditions and meals were given 0.5 and 4 hours later. Induction with ATG. Co-meds: cyclosporine doses to maintain morning blood C0 of 150-200mcg/L.
* Groups: Index (n=60), Test (n=10)
* Patient population: stable renal function with Scr<220 mcgmol/L; 
- Index group: median(range): age(yrs)=46(18-71), wt(kg)=64(39-97), ht(cm)=167(144-190), ast(U/L)=14(6-30), alt(U/L)=13(5-36), scr(mcgmol/L)=115(69-191), bili(mcgmol/L)=8(3-24), corticosteroids=29/60, reference AUC(mg*hr/L)=54(28-73)
- Test group: age=47(27-66), wt=62(50-130), ht=169(155-184), ast=14(9-27), alt=10(7-25), scr=130(78-217), bili=10(3-16), corticosteroid=3/10, reference AUC=42(26-65)


### Sampling strategy
* 20 patients had full PK study involving 10 blood samples drawn before C0, 20min, 40min, and 1, 1.5, 2, 3, 4, 6, 9 hrs after the morning dose. In 50 patients, blood samples were obtained before C0 and 2 hrs after C2 and at 2 other randomly selected time points. 


### Model structure
* 2-compartment model with zero-order absorption
* The index group was used to build the popPK model using NONMEM. This group included 10 patients randomly selected from the full-PK group and 50 patients from the sparse sampling group. The last 10 patients of the full-PK group were used as validation.
* An exponential error model was used to describe the IIV in the parameters.
* Covariate analysis showed that body weight was positively correlated to CL/F.
* Final model parameters: CL/F(L/hr)=0.246*wt, Vc/F(L)=36, Vp/F(L)=137, Q(L/hr)=25.9, D1(hr)=0.69; D1=duration of absorption, entering as ALAG_GUT below
- CV%: ECL=28.2, EVc=63.1, EQ=44.7, ED1=10.9
* Enterohepatic recirculation: secondary peaks were observed but relatively small so authors opted not to include EHPC 


### Simulated dosing regimen
* MMF 1g PO Q12H, addl=9
...
model2 <- models$use("02_mpa_guellec_adults")
...
sim_01 <- model2 %>% data_set(demog1) %>%
  carry.out(time, dose, amt, cmt, evid, ii, addl) %>% 
  mrgsim(start=0.1, end=12, delta=0.05) %>% as.data.frame() %>% filter(CP>=LLOQ)

sim_02 <- model2 %>% data_set(demog2) %>%
  carry.out(time, dose, amt, cmt, evid, ii, addl) %>% 
  mrgsim(start=0.1, end=12, delta=0.05) %>% as.data.frame() %>% filter(CP>=LLOQ)

sim_03 <- model2 %>% data_set(demog3) %>%
  carry.out(time, dose, amt, cmt, evid, ii, addl) %>% 
  mrgsim(start=0.1, end=12, delta=0.05) %>% as.data.frame() %>% filter(CP>=LLOQ)

sim_05 <- model2 %>% data_set(demog5) %>%
  carry.out(time, dose, amt, cmt, evid, ii, addl) %>% 
  mrgsim(start=0.1, end=12, delta=0.05) %>% as.data.frame() %>% filter(CP>=LLOQ)

sim_13 <- model2 %>% data_set(demog13) %>%
  carry.out(time, dose, amt, cmt, evid, ii, addl) %>% 
  mrgsim(start=0.1, end=12, delta=0.05) %>% as.data.frame() %>% filter(CP>=LLOQ)
...
sim_04 <- model2 %>% data_set(demog4) %>%
  carry.out(time, dose, amt, cmt, evid, ii, addl) %>% 
  mrgsim(start=0.1, end=12, delta=0.05) %>% as.data.frame() %>% filter(CP>=LLOQ)

sim_07 <- model2 %>% data_set(demog7) %>%
  carry.out(time, dose, amt, cmt, evid, ii, addl) %>% 
  mrgsim(start=0.1, end=12, delta=0.05) %>% as.data.frame() %>% filter(CP>=LLOQ)

sim_21 <- model2 %>% data_set(demog21) %>%
  carry.out(time, dose, amt, cmt, evid, ii, addl) %>% 
  mrgsim(start=0.1, end=12, delta=0.05) %>% as.data.frame() %>% filter(CP>=LLOQ)

sim_22 <- model2 %>% data_set(demog22) %>%
  carry.out(time, dose, amt, cmt, evid, ii, addl) %>% 
  mrgsim(start=0.1, end=12, delta=0.05) %>% as.data.frame() %>% filter(CP>=LLOQ)

sim_23 <- model2 %>% data_set(demog23) %>%
  carry.out(time, dose, amt, cmt, evid, ii, addl) %>% 
  mrgsim(start=0.1, end=12, delta=0.05) %>% as.data.frame() %>% filter(CP>=LLOQ)
